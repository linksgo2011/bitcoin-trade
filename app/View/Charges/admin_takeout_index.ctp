<div class="block span12">
    <div class="block-heading" data-toggle="collapse" data-target="#tablewidget">提现记录</div>
        ...
        <table class="table">
            ...
            <?php foreach ($data as $key => $one): ?>
                <tr>
                    ...
                    <td><?php echo $one['Charge']['address']; ?></td>
                    <td><?php echo date('Y-m-d H:i:s',$one['Charge']['created']); ?></td>
                    <td>
                        <?php if ($one['Charge']['active'] == 0): ?>
                            <?php echo $this->Html->link('处理',array('action'=>'takeout_deal','admin'=>true,$one['Charge']['id'])); ?>
                        <?php endif ?>
                    </td>
                </tr>
            <?php endforeach ?>
